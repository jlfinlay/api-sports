...
@Slf4j
@Service
public class UpperScoreService {
    ...
    public Response upperScore(ParamVO param, String timestamp) {
        log.info("用户上分,入参:param:{}",param);

        /** 添加分布式锁,防止并发 **/
        String redisKey = RedisKeyPrefix.getUserUpperScore(param.getAgent() + "_" + param.getSiteCode() + "_" + param.getAccount());
        try {
            if (!redisClient.setDistributedLock(redisKey, redisKey, 5)) {
                throw new GlobeException(GlobeException.FAIL_LOCK_OFTEN, "操作频繁");
            }
            verifyUpperScoreParam(param,timestamp);
            ...
            Response<ProcCallBackVO> response = openApiService.upperScore(upperScoreVo);
            log.info("用户上分返回参数:{}",response);
            return response;
     ...
     *  上分参数校验
     */
    private void verifyUpperScoreParam(ParamVO param, String timestamp) throws GlobeException {
